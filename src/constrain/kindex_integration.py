...
Provides context enrichment on session start and artifact publishing on
session completion via kindex's knowledge graph. All methods are silent
no-ops when kindex is not installed or inaccessible.

Usage from constrain::

    from constrain.kindex_integration import kindex

    context = kindex.fetch_context("auth service constraints")
    kindex.publish_constraints(yaml_content, tags=["auth"])
    kindex.close()
"""
...
def publish_constraints(constraints_yaml: str, tags: list[str] | None = None) -> int:
    """Parse constraints.yaml and publish each constraint as a node.

    Returns number of constraints published.
    """
    if not is_available() or not constraints_yaml.strip():
        return 0
    try:
        import yaml

        data = yaml.safe_load(constraints_yaml)
        ...
        for c in data.get("constraints", []):
            ...
            tier = c.get("classification_tier")
            if tier:
                node_tags.append(tier.lower())
            for comp in c.get("affected_components", []):
                node_tags.append(comp)
            publish_node(
                title=f"Constraint {cid}: {boundary} — {condition}",
                content=content,
        ...
        logger.debug("Kindex constraint publish failed: %s", e)
        return 0


def publish_components(component_map_yaml: str, tags: list[str] | None = None) -> int:
    """Parse component_map.yaml and publish each component."""
    if not is_available() or not component_map_yaml.strip():
        return 0
    try:
        import yaml

        data = yaml.safe_load(component_map_yaml)
        if not isinstance(data, dict):
            return 0
        count = 0
        ...
        logger.debug("Kindex component publish failed: %s", e)
        return 0
...
def read_kin_config(directory: Path) -> dict:
    ...
    if config_path.exists():
        try:
            ...
            return {}
    # Legacy: .kin as a plain YAML file
