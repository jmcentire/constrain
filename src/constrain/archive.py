...
When starting a new session, any artifacts from a previous session are
moved into a slug-named subdirectory under .constrain/archive/ to keep
the working directory clean.

    .constrain/archive/auth-service/
        prompt.md
        constraints.yaml
        trust_policy.yaml
        component_map.yaml
...
The slug is derived from file content (e.g., the system name in prompt.md
or a ``system`` key in constraints.yaml). All files from the same session
share one slug for coherence. Numeric suffixes handle collisions.
"""
